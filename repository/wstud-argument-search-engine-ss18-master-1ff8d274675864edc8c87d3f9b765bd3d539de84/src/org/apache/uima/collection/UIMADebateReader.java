...
/**
 * Reads all debate-portals from a given collection of plain text files 
 ...
 * @author jonas.dorsch
 */
public class UIMADebateReader extends CollectionReader_ImplBase {
	...
	/**
	 * Divides Each Discussion in every Portal
	 */
  		...
  		if (directory.exists() && directory.isDirectory()){
	  		...
	  		for (int i = 0; i < allFiles.length; i++) {
	  			if (allFiles[i].isDirectory() && includeSubdirs){
	  				...
	  				System.out.println(allFiles[i] + " read");
	  				String discussion = "";
	  				Scanner s = new Scanner( new File(allFiles[i].getPath())); //still needed
	  				boolean x = true;
	  				boolean disc = false;
	  				String tester = "";
	  				while(x) {
	  					...
	  					if(tester.equals(DIVISOR)
	  							|| tester.equals(DIVISOR2)) {
	  						disc ^= true;
	  						if(!disc) {	
