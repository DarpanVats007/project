 ...
 *
 */
public class DebatewiseAE extends JCasAnnotator_ImplBase {
	...
	public void process(JCas jcas) {
		...
		if(checkPortal.toLowerCase().contains("debatewise.org")) {
			...
			for (int i=0; i<lines.length; i++) {
				...
				if(keyWord.equals(URL) && !(split[1].isEmpty())) {
					...
					titleLine = i;
					int cut = split[1].indexOf(" - DebateWise");
					...
					int trimConclusion = conclusion.indexOf(" - DebateWise");
					int keyWordConclusion = TITLE.length() + 1;
					...
					ArgumentativeDiscourseUnit adu1 = 
							new ArgumentativeDiscourseUnit(jcas, lineOffsets.get(titleLine) + keyWordConclusion, lineOffsets.get(titleLine) + trimConclusion);
					if(!(Functions.allreadyExists(jcas, adu1))) {
						adu1.addToIndexes();
					}
					adu2 = 
							new ArgumentativeDiscourseUnit(jcas, lineOffsets.get(i) + keyWord.length() + 1, lineOffsets.get(i+1)-1);
					adu2.addToIndexes();
					...
					//Ich verstehe nicht was ich mir hierbei gedacht habe.... später überlegen :D
					...
					ArgumentativeDiscourseUnit adu3 = 
							new ArgumentativeDiscourseUnit(jcas, lineOffsets.get(i) + keyWord.length() + 1,lineOffsets.get(i) + lines[i].length());
					adu3.addToIndexes();
